...
def data_dict(s):
    '''
    dictionary type of city and monthly rain info
    key is city name
    values are list type of monthly rain info ["month", "rain"]
    '''
    t_data = data.split("\n")
    data_dic = {}
    t = [line.split(":") for line in t_data]
    
    for line in t:
        rain = line[1].split(",")
        data_dic[line[0]] = [r.split(" ")for r in rain]
    return data_dic    
...
def mean(town, s):
    # your code
    rain_info = data_dict(s) # dictionary type of city and monthly rain info
